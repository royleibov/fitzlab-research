...
omega_d_list = np.linspace(0.8 * args_template['omega_0'], 1.2 * args_template['omega_0'], 50)
...
for omega_d in tqdm(omega_d_list, desc='Sweeping over driving frequencies'):
    ...
    def driving_frequency(t, args):
        ...
        return omega_R * strength * np.exp(omega_d * t * 1j)
    ...
    assert np.allclose(results.expect[0] + results.expect[1], 1)
